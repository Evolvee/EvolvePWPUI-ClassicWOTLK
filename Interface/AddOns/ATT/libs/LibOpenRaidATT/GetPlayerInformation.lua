...
local CONST_TALENT_VERSION_CLASSIC_ERA = 1
local CONST_TALENT_VERSION_CLASSIC = 2
...
openRaidLib.LIB_OPEN_RAID_WEAPON_ENCHANT_IDS = {
...
openRaidLib.LIB_OPEN_RAID_DEATHKNIGHT_RUNEFORGING_ENCHANT_IDS = {
...
openRaidLib.LIB_OPEN_RAID_ENCHANT_SLOTS = {
    --[INVSLOT_NECK] = true,
    [INVSLOT_BACK] = true,
    [INVSLOT_CHEST] = true,
...
openRaidLib.LIB_OPEN_RAID_PLAYERCOOLDOWNS = {}
...
function openRaidLib.GetTalentVersion()
    ...
    if (buildInfo >= 1 and buildInfo <= 20000) then --vanilla tbc wotlk cataclysm
        return CONST_TALENT_VERSION_CLASSIC_ERA
    end
    
    if (buildInfo >= 20000 and buildInfo <= 40000) then --vanilla tbc wotlk cataclysm
...
function openRaidLib.UnitInfoManager.GetPlayerTalentsAsPairsTable()
    ...
    elseif (talentVersion == CONST_TALENT_VERSION_LEGION) then
     ...
     elseif (talentVersion == CONST_TALENT_VERSION_CLASSIC) then
        ...
        for i = 1, 4 do
            for j = 1, 31 do
                local talentId, _, _, selected, rank = GetTalentInfo(i, j, isInspect, false, talentspec)
                if talentId and selected then talentsPairs[talentId] = true end
            end
        end
    elseif (talentVersion == CONST_TALENT_VERSION_CLASSIC) then
        local talentspec = GetActiveTalentGroup(isInspect)
     ...
     elseif (talentVersion == CONST_TALENT_VERSION_CLASSIC_ERA) then
        for i = 1, 4 do
            for j = 1, 31 do
                local talentId, _, _, selected, rank = GetTalentInfo(i, j, isInspect, false)
                if talentId and selected then talentsPairs[talentId] = true end
            end
        end     
    end
...
function openRaidLib.UnitInfoManager.GetPlayerTalents()
    ...
    elseif (talentVersion == CONST_TALENT_VERSION_LEGION) then
        ...
        for i = 1, 4 do
            for j = 1, 31 do
                local talentId, _, _, selected, rank = GetTalentInfo(i, j, isInspect, false, talentspec)
                if talentId and selected then talents[talentId] = true end
            end
        end
       elseif (talentVersion == CONST_TALENT_VERSION_CLASSIC_ERA) then
        --talents = {0, 0, 0, 0, 0, 0, 0}
...
function openRaidLib.GearManager.GetPlayerWeaponEnchant()
    ...
    if (openRaidLib.LIB_OPEN_RAID_WEAPON_ENCHANT_IDS[mainHandEnchantId]) then
        weaponEnchant = 1

    elseif(openRaidLib.LIB_OPEN_RAID_WEAPON_ENCHANT_IDS[offHandEnchantId]) then
        weaponEnchant = 1
    end
    ...
    for equipmentSlotId = 1, 17 do
        ...
        if (itemLink) then
            ...
            --enchant
                ...
                local enchantAttribute = openRaidLib.LIB_OPEN_RAID_ENCHANT_SLOTS[equipmentSlotId]
                local nEnchantId = 0
                ...
                if (enchantAttribute) then --this slot can receive an enchant
                    ...
                    if (nEnchantId < 6300 and not openRaidLib.LIB_OPEN_RAID_DEATHKNIGHT_RUNEFORGING_ENCHANT_IDS[nEnchantId]) then
            ...
            for entryOffset = offset, tabEnd - 1 do
                ...
                if (spellId) then
                    if (spellType == "SPELL") then
                      -- if isDF then spellId = C_SpellBook.GetOverrideSpell(spellId) end
                        local spellName = GetSpellInfo(spellId)
                        local bIsPassive = IsPassiveSpell(spellId)
                        if (spellName and not bIsPassive) then
                            completeListOfSpells[spellId] = true
                        end
                    end
...
local updateCooldownAvailableList = function()
    table.wipe(openRaidLib.LIB_OPEN_RAID_PLAYERCOOLDOWNS)
    ...
    for spellID, spellData in pairs(LIB_OPEN_RAID_COOLDOWNS_INFO) do
        ...
        if (spellData.class == playerClass or passRaceId or CONST_ISITEM_BY_TYPEID[spellData.type]) then --need to implement here to get the racial as racial cooldowns does not carry a class
            ...
            if (spellBookSpellList[spellID] or CONST_ISITEM_BY_TYPEID[spellData.type]) then
              --  openRaidLib.LIB_OPEN_RAID_PLAYERCOOLDOWNS[spellID] = spellData
            end
        end
    end
    --]]
    ...
    for abilityIndex, abilityTable in pairs(ATTdbs.dbRacials) do
       -- if spellBookSpellList[abilityTable.ability] then
            local timeLeft, charges, startTimeOffset, duration, auraDuration = openRaidLib.CooldownManager.GetPlayerCooldownStatus(abilityTable.ability)
            openRaidLib.LIB_OPEN_RAID_PLAYERCOOLDOWNS[abilityTable.ability] = {duration = auraDuration or 0,  specs = {},talent = false,charges = charges or 0, class = playerClass};
       -- end
    end

   local specid = openRaidLib.GetPlayerSpecId()
   local specSpells = not isDF and ATTdbs.dbImport[playerClass] or ATTdbs.dbImport[playerClass][tostring(specid)]
   if specSpells then
    ...
    for abilityIndex, abilityTable in pairs(ATTdbs.ShareCD) do
         if isDF and spellBookSpellList[abilityIndex] then
             local timeLeft, charges, startTimeOffset, duration, auraDuration = openRaidLib.CooldownManager.GetPlayerCooldownStatus(abilityIndex)
             openRaidLib.LIB_OPEN_RAID_PLAYERCOOLDOWNS[abilityIndex] = {duration = auraDuration or 0,  specs = {},talent = false,charges = charges or 0, class = playerClass};
         end
     end
end
...
function openRaidLib.CooldownManager.GetPlayerCooldownList()
    ...
    --if (IsDragonflight) then
        --this fill the global openRaidLib.LIB_OPEN_RAID_PLAYERCOOLDOWNS
        updateCooldownAvailableList()

        --get the player specId
            --get the cooldowns for the specialization
            local playerCooldowns = openRaidLib.LIB_OPEN_RAID_PLAYERCOOLDOWNS
            if (not playerCooldowns) then
                openRaidLib.DiagnosticError("CooldownManager|GetPlayerCooldownList|openRaidLib.LIB_OPEN_RAID_PLAYERCOOLDOWNS is nil")
                return {}, {}
            end
            ...
            return cooldowns or {}, cooldownsHash or {}
end
...
local getAuraDuration = function(spellId)
    ...
    if (not openRaidLib.LIB_OPEN_RAID_PLAYERCOOLDOWNS[spellId]) then
        --local spellname = GetSpellInfo(spellId)
        --print("spell not found:", spellname)
        return 0
    end
    --spellId = customBuffDuration or spellId --can't replace the spellId by customBuffDurationSpellId has it wount be found in openRaidLib.LIB_OPEN_RAID_PLAYERCOOLDOWNS
    ...
    if (bIsNewUnitAuraAvailable) then
        ...
        if (auraDurationTime == 0) then --if the buff wasn't found, attempt to get the duration from the file
            return openRaidLib.LIB_OPEN_RAID_PLAYERCOOLDOWNS[spellId].duration or 0
        end
