...
--default values
    ...
    local CONST_CVAR_TEMPCACHE = "LibOpenRaidTempCacheATT"
    local CONST_CVAR_TEMPCACHE_DEBUG = "LibOpenRaidTempCacheDebugATT"
    ...
    local CONST_COMM_PREFIX = "LRSATT"
...
--~comms
    ...
    function openRaidLib.commHandler.OnReceiveComm(self, event, prefix, text, channel, sender, target, zoneChannelID, localID, name, instanceID)
        ...
        if prefix == CONST_COMM_PREFIX then SENDER_HAS_ATT[sender] = true end

        if (prefix == CONST_COMM_PREFIX or (prefix == "LRS" and not SENDER_HAS_ATT[sender]) ) then
        ...
        if (flags) then
            if (bit.band(flags, CONST_COMM_SENDTO_PARTY)) then --send to party
                if (IsInGroup() and not IsInRaid()) then
                    ...
                    if data == "F" and not hasLOR then sendDataLR(dataEncoded, IsInGroup(LE_PARTY_CATEGORY_INSTANCE) and "INSTANCE_CHAT" or "PARTY") end
                end
            end
            ...
            if (bit.band(flags, CONST_COMM_SENDTO_RAID)) then --send to raid
                if (IsInRaid()) then
                    ...
                    if data == "F" and not hasLOR then sendDataLR(dataEncoded, IsInRaid(LE_PARTY_CATEGORY_INSTANCE) and "INSTANCE_CHAT" or "RAID") end
                end
            end
...
--internally, each module can register events through the internal callback to be notified when something happens in the game
    ...
    local eventFrame = _G.OpenRaidLibFrameATT
    if (not eventFrame) then
        eventFrame = CreateFrame("frame", "OpenRaidLibFrameATT", UIParent)
    end
    ...
    local eventFunctions = {
        ...
        ["GROUP_JOINED"] = function(...)
            ...
            if instanceType == "arena" then
                openRaidLib.UnitInfoManager.OnPlayerLeaveGroup()
                openRaidLib.RequestAllData()
                openRaidLib.UpdateUnitIDCache()
                --openRaidLib.mainControl.OnEnterGroup()
            end 
        end,
...
--API Calls
    ...
    function openRaidLib.GetCooldownStatusFromUnitSpellID(unitId, spellId)
        ...
        return calculatePercent(timeOffset, duration, updateTime, charges) --here
    end
...
--internals
    ...
    function openRaidLib.CooldownManager.OnPlayerCast(event, spellId, isPlayerPet) --~cast
        ...
        if (openRaidLib.LIB_OPEN_RAID_PLAYERCOOLDOWNS[spellId]) then --check if the casted spell is a cooldown the player has available
            local playerName = UnitName("player")
