...
local function CreateIcon(anchor)
    ...
    icon.Start = function(sentCD , nextcharge, rate)
        ...
        if icon.ability and icon.abilityID and anchor.class and db.alertCD[anchor.class][icon.abilityID] and nextcharge ~= 5 then PlaySound(8959, "Master"); end
        if anchor.class and db.alertCDtext[anchor.class][icon.abilityID] then
...
function ATT:MergeTable(class)
    ...
    local iconOrder = db.iconOrder[class]
       ...
       button:SetHitRectInsets(0, -_G[button:GetName() .. "Text"]:GetStringWidth() - 1, 0, 0)
        ...
        orderbtn:SetScript("OnClick", function(self)
            ...
            local order = panel:MakeSlider(
                ...
                'step', 1,
                'default', 1,
                'current', tonumber(db.iconOrder[db.classSelected][id]) or 10,
                'setFunc', function(value) if value ~= db.iconOrder[db.classSelected][id] then db.iconOrder[db.classSelected][id] = tonumber(string.format("%.1d",value)) end end,
                'currentTextFunc', function(value) return tonumber(string.format("%.1d",value)) end)
            ...
            local alertCD = panel:MakeToggle(
                ...
                'default', false,
                'getFunc', function() return db.alertCD[db.classSelected][id] end,
                'setFunc', function(value) db.alertCD[db.classSelected][id] = value;  end)
           ...
           local alertCDtext = panel:MakeToggle(
                ...
                'default', false,
                'getFunc', function() return db.alertCDtext[db.classSelected][id] end,
                'setFunc', function(value) db.alertCDtext[db.classSelected][id] = value;  end)
